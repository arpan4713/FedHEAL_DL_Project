...
# from datasets import Priv_NAMES as DATASET_NAMES, get_prive_dataset
# from models import MyFederatedModel, get_model, get_all_models
# from utils.args import add_management_args
# from utils.training import train
# from utils.best_args import best_args
# from utils.conf import set_random_seed
# from utils import calculate_consistency, aggregate_updates
# from fed_heal import FedHEAL  # Assuming FedHEAL class is moved to fed_heal.py for clarity
...
#     # FedHEAL-specific parameters
#     parser.add_argument('--wHEAL', type=int, default=1, help='The CORE of the FedHEAL decides whether to add HEAL to other FL method')
#     parser.add_argument('--threshold', type=float, default=0.3, help='threshold of HEAL')
#     parser.add_argument('--beta', type=float, default=0.4, help='momentum update beta')

#     # Dataset client counts
#     parser.add_argument('--mnist', type=int, default=5, help='Number of mnist clients')
#     parser.add_argument('--usps', type=int, default=5, help='Number of usps clients')
#     parser.add_argument('--svhn', type=int, default=5, help='Number of svhn clients')
#     parser.add_argument('--syn', type=int, default=5, help='Number of syn clients')
...
#     # Set number of threads and add custom management args
#     torch.set_num_threads(4)
#     add_management_args(parser)

#     # Parse arguments and load best configuration
#     args = parser.parse_args()
#     best = best_args[args.dataset][args.model]
#     for key, value in best.items():
#         setattr(args, key, value)
...
#     # Add meta-information for the current run
#     args.conf_jobnum = str(uuid.uuid4())
#     args.conf_timestamp = str(datetime.datetime.now())
#     args.conf_host = socket.gethostname()

#     # Load dataset and backbone models
#     priv_dataset = get_prive_dataset(args)
#     backbones_list = priv_dataset.get_backbone(args.parti_num, None)
#     global_model = get_model(backbones_list, args, priv_dataset.get_transform())
#     args.arch = global_model.nets_list[0].name

#     # Initialize clients (assuming a Client class exists in your code)
#     clients = [Client(args, model) for model in backbones_list]  # Adjust Client class usage as needed

#     # Initialize FedHEAL with clients and global model
#     fed_heal = FedHEAL(clients=clients, global_model=global_model, lr=args.lr)

#     # Print configuration summary
#     print(f'Starting Federated Training for {args.model} with {args.parti_num} clients, '
#           f'dataset={args.dataset}, communication_epoch={args.communication_epoch}, local_epoch={args.local_epoch}')

#     # Run the FedHEAL training
#     fed_heal.train(num_rounds=args.communication_epoch)
